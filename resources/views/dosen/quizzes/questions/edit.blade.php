    ...
    <div class="p-6 max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">
                Edit Question - {{ $question->quiz->title }}
            </h2>
            <a href="{{ route('dosen.quizzes.questions.index', $question->quiz) }}" 
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6" x-data="{ type: '{{ old('question_type', $question->question_type) }}' }">
            <form action="{{ route('dosen.quizzes.questions.update', $question) }}" method="POST">
                @csrf
                @method('PUT')
