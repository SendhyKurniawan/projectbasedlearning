...
<x-app-layout>
    <div class="max-w-3xl mx-auto"
        ...
        <nav class="flex items-center gap-2 text-xs font-bold text-on-surface-variant/60 uppercase tracking-widest mb-3">
            <a href="{{ route('dosen.conferences.index', $conference->course_id) }}" class="hover:text-primary transition-colors">Live Conference</a>
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-3xl p-8 md:p-10 shadow-sm">
            <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs font-bold text-on-surface-variant/70 uppercase tracking-widest">Moderator</span>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-outline-variant flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                <a href="{{ route('dosen.conferences.index', $conference->course_id) }}" class="inline-flex items-center gap-2 text-sm font-bold text-on-surface-variant hover:text-primary transition-colors">
                ...
                <form x-ref="endForm" method="POST" action="{{ route('dosen.conferences.end', $conference) }}" class="hidden">
                    @csrf
                </form>
            </div>
        </div>
