...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .status-pill.recording { background: rgba(234,67,53,0.15); border: 1px solid rgba(234,67,53,0.35); color: #f28b82; }
        ...
        #participant-count { font-size: 12px; color: #9aa0a6; }
        ...
        /* ---- Main video area ---- */
        #video-section { flex: 1; display: flex; overflow: hidden; min-width: 0; gap: 8px; }
        ...
        /* Panels use display:none via panel-hidden class - NO inline display style on children */
        ...
        .chat-input-box { position: relative; background: #3c4043; border-radius: 16px; overflow: hidden; }
        ...
        #participants-footer { display: flex; justify-content: space-around; padding: 10px 16px; border-top: 1px solid rgba(255,255,255,0.08); flex-shrink: 0; }

        /* ---- Control Bar ---- */
        .controlbar-left { display: flex; align-items: center; gap: 8px; min-width: 200px; }
        .controlbar-center { display: flex; align-items: center; gap: 6px; }
        .controlbar-right { display: flex; align-items: center; gap: 6px; justify-content: flex-end; min-width: 200px; }
        ...
        .ctrl-active-panel { background: rgba(26,115,232,0.2); color: #8ab4f8; }
        .ctrl-end { display: flex; align-items: center; gap: 6px; padding: 0 16px; height: 48px; border-radius: 24px; background: #ea4335; color: #fff; border: none; cursor: pointer; font-size: 13px; font-weight: 600; transition: background 0.15s, transform 0.1s; }
        .ctrl-end:hover { background: #d93025; transform: scale(1.03); }
        .ctrl-end svg { width: 20px; height: 20px; }
        .ctrl-leave { display: flex; align-items: center; gap: 6px; padding: 0 16px; height: 48px; border-radius: 24px; background: #3c4043; color: #e8eaed; border: 1px solid #5f6368; cursor: pointer; font-size: 13px; font-weight: 600; transition: background 0.15s, transform 0.1s; }
        .ctrl-leave:hover { background: #5f6368; }
        .ctrl-leave svg { width: 20px; height: 20px; }
        ...
        /* More menu */
        #more-menu { position: absolute; bottom: 66px; left: 50%; transform: translateX(-20%); background: #3c4043; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); padding: 6px 0; min-width: 200px; z-index: 100; }
        ...
        .meeting-info-badge { display: flex; flex-direction: column; min-width: 0; }
        .meeting-room-id { font-size: 13px; font-weight: 500; color: #e8eaed; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 120px; }
        #copy-link-text { font-size: 11px; color: #9aa0a6; cursor: pointer; transition: color 0.15s; }
        #copy-link-text:hover { color: #8ab4f8; }
    ...
    <div id="topbar">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="meeting-meta">{{ $conference->course->nama_matkul }} &bull; Dosen</div>
            </div>
        </div>
        ...
        <div id="side-panel-wrapper" class="panel-hidden">
            ...
            <div id="panel-participants" class="panel-hidden">
                ...
                <div style="padding:0 12px 8px;flex-shrink:0;">
                    <button onclick="ConferenceUI.copyLink()" style="width:100%;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;background:#3c4043;color:#fff;border:none;cursor:pointer;font-size:13px;transition:background 0.15s;" onmouseover="this.style.background='#4a4d51'" onmouseout="this.style.background='#3c4043'">
                        <svg width="16" height="16" fill="#1a73e8" viewBox="0 0 24 24"><path d="M15 7H9c-2.76 0-5 2.24-5 5s2.24 5 5 5h6c2.76 0 5-2.24 5-5s-2.24-5-5-5zm0 8H9c-1.66 0-3-1.34-3-3s1.34-3 3-3h6c1.66 0 3 1.34 3 3s-1.34 3-3 3z"/></svg>
                        <span id="copy-link-text">Salin Tautan</span>
                        <svg width="14" height="14" fill="#9aa0a6" viewBox="0 0 24 24" style="margin-left:auto"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>
                    </button>
                </div>
    ...
    <div id="bottombar">
        ...
        <div class="controlbar-left">
            <div class="meeting-info-badge">
                <div class="meeting-room-id">{{ $conference->title }}</div>
                <div id="copy-link-text" onclick="ConferenceUI.copyLink()" style="font-size:11px;color:#9aa0a6;cursor:pointer;transition:color 0.15s;" onmouseover="this.style.color='#8ab4f8'" onmouseout="this.style.color='#9aa0a6'">Salin Tautan</div>
            </div>
        </div>
        ...
        <div class="controlbar-center" style="position:relative;">
            ...
            <div style="position:relative;">
                ...
                <div id="more-menu" class="hidden">
                    ...
                    <button class="more-menu-item" onclick="ConferenceUI.copyLink();document.getElementById('more-menu').classList.add('hidden');">
            ...
            {{-- Leave --}}
            <button class="ctrl-leave" onclick="ConferenceRoom.disconnect('{{ route('dosen.conferences.index', $conference->course) }}')" title="Keluar dari ruangan">
                <svg fill="currentColor" viewBox="0 0 24 24"><path d="M10.09 15.59L11.5 17l5-5-5-5-1.41 1.41L12.67 11H3v2h9.67l-2.58 2.59zM19 3H5a2 2 0 00-2 2v4h2V5h14v14H5v-4H3v4a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2z"/></svg>
                Keluar
            </button>

            {{-- End Session --}}
            <form method="POST" action="{{ route('dosen.conferences.end', $conference) }}" onsubmit="return ConferenceRoom.confirmEnd(event)" style="display:contents;">
                @csrf
                <button type="submit" class="ctrl-end" title="Akhiri sesi untuk semua peserta">
...
<script>
// Chat helpers accessible from Blade-rendered onclick handlers
const ChatHelper = {
    addEmoji(emoji) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Init conference room
    ConferenceRoom.init(
        '{{ route('dosen.conferences.token', $conference) }}',
