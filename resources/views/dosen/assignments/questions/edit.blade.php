        ...
        <div class="flex flex-wrap items-end justify-between gap-4">
            <div>
                <nav class="flex items-center gap-2 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                    <a href="{{ route('dosen.assignments.index', $question->assignment->course) }}" class="hover:text-primary transition-colors">Assignments</a>
                    <span class="material-symbols-outlined text-xs">chevron_right</span>
                    <a href="{{ route('dosen.assignments.questions.index', $question->assignment) }}" class="hover:text-primary transition-colors truncate max-w-[140px]">{{ $question->assignment->title }}</a>
                    <span class="material-symbols-outlined text-xs">chevron_right</span>
                    <span class="text-primary">Edit Pertanyaan</span>
                </nav>
                <h1 class="font-headline text-3xl font-extrabold tracking-tight text-on-surface">Edit Pertanyaan</h1>
                <p class="mt-1 text-sm text-on-surface-variant">{{ $question->assignment->title }}</p>
            </div>
            <a href="{{ route('dosen.assignments.questions.index', $question->assignment) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-outline-variant/30 text-on-surface text-sm font-bold hover:bg-surface-container-low transition">
