<x-app-layout>
 ...
 <div class="bg-surface-container-lowest rounded-xl shadow-sm p-6" x-data="{ type: '{{ old('question_type', 'pilihan_ganda') }}' }">
 ...
 class="w-full rounded-lg border-outline-variant/30 focus:ring-blue-500 focus:border-blue-500">{{ old('question_text') }}</textarea>
 @error('question_text')
 ...
 class="w-full rounded-lg border-outline-variant/30 focus:ring-blue-500 focus:border-blue-500">
 <p class="text-xs text-on-surface-variant mt-1">Bobot nilai pertanyaan ini.</p>
 </div>
 </div>
 ...
 @endfor
 <p class="text-xs text-on-surface-variant">Select the radio button for the correct answer.</p>
 @error('options')
 <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
 @enderror
 </div>
