...
<x-app-layout>
 ...
 <h1 class="font-headline text-3xl font-extrabold tracking-tight text-on-surface">Edit Tugas</h1>
 ...
 * Setelah menyimpan, Anda akan diarahkan untuk mengisi pertanyaan.
 ...
 :required="type === 'tugas'"
 :disabled="type !== 'tugas'">
 <option value="pdf" {{ ($assignment->submission_format ?? 'pdf') === 'pdf' ? 'selected' : '' }}>File Upload (PDF, DOCX, ZIP, dll)</option>
 <option value="url" {{ ($assignment->submission_format ?? 'pdf') === 'url' ? 'selected' : '' }}>Link URL (Github, GDrive, Youtube, dll)</option>
 </select>
 ...
 @if($hasSubmission)
 <div class="bg-warning-light/30 border border-warning/20 text-on-surface-variant text-xs p-3 rounded-lg mb-3">
 <span class="font-bold">Terkunci:</span> Pengaturan kelompok tidak bisa diubah karena sudah ada pengumpulan.
 </div>
 @endif
 <label class="inline-flex items-center cursor-pointer">
 <input type="checkbox" name="is_group" value="1" class="w-4 h-4 text-primary bg-surface-container-low border-outline-variant/30 rounded focus:ring-primary focus:ring-2" x-model="is_group" :disabled="type !== 'tugas' || {{ $hasSubmission ? 'true' : 'false' }}">
 ...
 value="{{ old('max_group_size', $assignment->max_group_size ?? 5) }}" min="2" max="20"
 class="w-full border-outline-variant/30 text-on-surface rounded-md shadow-sm focus:border-primary focus:ring-primary"
 :required="is_group" :disabled="!is_group || {{ $hasSubmission ? 'true' : 'false' }}">
 ...
 <input type="radio" name="grading_mode" value="equal" x-model="grading_mode" class="mt-1 text-primary focus:ring-primary" :disabled="!is_group || {{ $hasSubmission ? 'true' : 'false' }}">
 <span>
 ...
 <input type="radio" name="grading_mode" value="individual" x-model="grading_mode" class="mt-1 text-primary focus:ring-primary" :disabled="!is_group || {{ $hasSubmission ? 'true' : 'false' }}">
 <span>
