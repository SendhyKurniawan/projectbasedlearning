<x-app-layout>
 ...
 <div>
 <h1 class="text-3xl font-bold text-on-surface">{{ $quiz->title }} - Attempt Details</h1>
 <p class="text-on-surface-variant mt-1">
 Student: {{ $attempt->mahasiswa->name }} <br>
 Finished at: {{ $attempt->finished_at->format('d M Y, H:i') }}
 </p>
 </div>
 <a href="{{ route('dosen.quizzes.attempts.index', $quiz) }}" class="px-4 py-2 bg-gray-600 hover:bg-surface-container-high text-white rounded-lg transition shadow-md">
 ...
 {{ $attempt->total_score ?? 0 }} <span class="text-lg text-on-surface-variant">/ {{ $quiz->questions->sum('score_weight') }}</span>
 </div>
 ...
 <span class="text-xs font-semibold text-error uppercase">Student Answer</span>
 @endif
 </div>
 @endforeach
 </div>
 ...
 <div class="mt-2">
 <p class="text-sm font-semibold text-on-surface-variant mb-1">Student Answer:</p>
