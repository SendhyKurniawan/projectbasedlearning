<x-app-layout>
 ...
 <div class="space-y-6">
 <div class="max-w-4xl mx-auto border-gray-100">
 <div class="bg-surface-container-lowest overflow-hidden shadow-sm rounded-2xl">
 <div class="p-6 pb-2 border-b border-gray-100 flex justify-between items-center">
 <h3 class="text-lg font-medium text-on-surface">Daftar Notifikasi</h3>
 @if(auth()->user()->unreadNotifications->count() > 0)
 <form action="{{ route('notifications.markAllRead') }}" method="POST">
 @csrf
 <button type="submit" class="px-4 py-2 bg-indigo-50 border border-indigo-200 rounded-md font-medium text-xs text-indigo-700 uppercase hover:bg-indigo-100 transition ease-in-out duration-150:bg-indigo-900">
 ...
 <div class="p-6 border-b border-gray-100 transition {{ $notification->unread() ? 'bg-blue-50/30' : '' }}">
 ...
 <div class="w-10 h-10 rounded-full bg-emerald-50/50 flex items-center justify-center text-green-600">
 <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
 </div>
 @elseif(($notification->data['type'] ?? '') === 'announcement')
 <div class="w-10 h-10 rounded-full bg-amber-50/50 flex items-center justify-center text-yellow-600">
 ...
 <span class="inline-block w-2.5 h-2.5 bg-blue-600 rounded-full mr-1" title="Belum Dibaca"></span>
 @endif
 ...
 <a href="{{ route('notifications.readAndRedirect', $notification->id) }}" class="text-sm font-medium text-primary hover:text-primary-container:text-indigo-300">
 Lihat Rincian &rarr;
 </a>
 @endif
 ...
 @csrf
 <button type="submit" class="text-sm font-medium text-on-surface-variant hover:text-on-surface-variant:text-outline">
 Tandai Sudah Dibaca
 </button>
 </form>
 @endif
