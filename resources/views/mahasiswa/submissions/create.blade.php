    ...
    <div class="max-w-4xl mx-auto space-y-10">
        ...
        <div>
            <nav class="flex items-center gap-2 text-xs font-bold text-on-surface-variant/60 uppercase tracking-widest mb-2 italic px-1">
                ...
                <span class="material-symbols-outlined text-[12px]">chevron_right</span>
                <span class="text-primary italic">Submit Assignment</span>
            </nav>
            <h1 class="text-4xl font-headline font-black text-on-surface italic uppercase tracking-tighter leading-tight">Pengumpulan Tugas</h1>
            <p class="text-on-surface-variant body-md mt-1 italic">Silakan lengkapi form di bawah ini untuk mengirimkan hasil pengerjaan anda.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            ...
            <div class="col-span-1 space-y-6">
                <div class="bg-surface-container-lowest rounded-[2rem] p-6 border border-outline-variant/10 shadow-sm relative overflow-hidden group">
                    ...
                    <div class="relative space-y-6">
                        <div>
                            <span class="text-[10px] font-black uppercase text-primary tracking-widest italic mb-2 block">Daftar Mata Kuliah</span>
                            <h3 class="text-lg font-black italic text-on-surface leading-tight uppercase tracking-tighter">{{ $assignment->course->nama_matkul }}</h3>
                        </div>
                        ...
                        <div class="bg-surface-container-low rounded-2xl p-4 space-y-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest italic leading-none opacity-60">Deadline</p>
                                    <p class="text-xs font-black italic text-on-surface">{{ $assignment->deadline->format('d M, H:i') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3">
                                <span class="material-symbols-outlined text-tertiary text-[20px]" style="font-variation-settings: 'FILL' 1;">military_tech</span>
                                <div>
                                    <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest italic leading-none opacity-60">Nilai Maksimal</p>
                                    <p class="text-xs font-black italic text-on-surface">{{ $assignment->max_score }} Poin</p>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($assignment->description)
                            <div>
                                <h4 class="text-[10px] font-black uppercase text-on-surface-variant tracking-widest italic mb-2 opacity-60">Instruksi:</h4>
                                <div class="text-xs text-on-surface-variant leading-relaxed italic opacity-90 line-clamp-6">
                ...
                <div class="p-6 bg-yellow-500/5 border border-yellow-500/20 rounded-[2rem] flex items-start gap-4">
                    ...
                    <p class="text-[11px] font-bold text-yellow-800 leading-relaxed italic">Pastikan seluruh file atau tautan yang dikirimkan dapat diakses oleh dosen pengampu.</p>
                </div>
            </div>
            ...
            <div class="col-span-1 md:col-span-2">
                @if($existing)
                    <div class="bg-surface-container-lowest rounded-[2.5rem] p-10 border border-outline-variant/10 shadow-sm text-center space-y-6 italic">
                        ...
                        <form action="{{ route('mahasiswa.submissions.store') }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-8">
                            ...
                            @if($assignment->submission_format === 'url')
                                ...
                                <div class="space-y-3">
                                    ...
                                    <p class="text-[10px] text-on-surface-variant/60 font-bold italic ml-2">Gunakan Github, Google Drive, atau platform lain yang representatif.</p>
                                    @error('url_link')
                                        <p class="text-error text-[11px] font-bold italic mt-1 ml-2 flex items-center gap-1">
                                            <span class="material-symbols-outlined text-sm">error</span> {{ $message }}
                                        </p>
                                    ...
                                    <div class="relative group cursor-pointer">
                                        ...
                                        <div class="w-full bg-surface-container-low border-2 border-dashed border-outline-variant/20 rounded-[2rem] p-8 text-center group-hover:border-primary/40 group-hover:bg-primary/5 transition-all relative z-10 overflow-hidden">
                                            <div class="relative z-10">
                                                ...
                                                <p class="text-sm font-black italic text-on-surface group-hover:text-primary">Klik atau seret file ke sini</p>
                                                <p class="text-[10px] text-on-surface-variant font-bold italic mt-1 opacity-60">Sistem mendukung format PDF, ZIP, RAR, atau Gambar</p>
                                            </div>
                                        </div>
                                    </div>
                                    @error('file')
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-4 pt-6">
                                <button type="submit" class="w-full md:flex-1 bg-primary text-on-primary font-black text-xs uppercase tracking-[0.2em] py-4 rounded-2xl hover:shadow-2xl hover:shadow-primary/30 active:scale-[0.98] transition-all flex items-center justify-center gap-2 italic">
                                    KUMPULKAN TUGAS
