    ...
    <div class="flex flex-col h-[calc(100vh-80px)] -mt-6">
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <section class="w-1/3 min-w-[380px] bg-surface-container-low border-r border-outline-variant/10 overflow-y-auto p-8 custom-scrollbar">
                <nav class="flex items-center gap-2 text-[10px] font-black font-headline text-on-surface-variant/60 uppercase tracking-widest mb-6 italic">
                    ...
                    <span class="text-primary italic">Code Exercise</span>
                </nav>

                <div class="flex items-center gap-2 mb-3">
                    <span class="px-2 py-0.5 bg-secondary-container text-on-secondary-container text-[8px] font-black rounded uppercase tracking-widest italic">Assignment #{{ $assignment->assignment_number }}</span>
                    <span class="text-on-surface-variant text-[10px] font-bold italic">• {{ $assignment->max_score }} Points Max</span>
                </div>
                
                <h2 class="text-3xl font-headline font-black text-on-surface italic uppercase tracking-tighter leading-tight mb-8">{{ $assignment->title }}</h2>
                ...
                @if($existing)
                    <div class="mb-8 p-5 bg-secondary/5 border border-secondary/20 rounded-2xl space-y-3 relative overflow-hidden">
                        ...
                        <h4 class="text-xs font-black text-secondary uppercase tracking-widest italic flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm">task_alt</span> SOLVED & SUBMITTED
                        </h4>
                        <div class="flex justify-between items-center text-on-surface italic">
                ...
                <div class="prose prose-slate prose-sm max-w-none space-y-8">
                    <div>
                        <h4 class="text-[10px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-4 italic opacity-60">PROBLEM STATEMENT</h4>
                        <div class="text-on-surface leading-loose italic">
                    ...
                    @if(!empty(data_get($assignment->exercise_config, 'hints', [])))
                        <div class="bg-primary/5 rounded-[2rem] p-6 border border-primary/10 relative overflow-hidden">
                            ...
                            <h4 class="text-[10px] font-black text-primary uppercase tracking-widest mb-4 italic">KEY REQUIREMENTS & HINTS</h4>
                            <ul class="space-y-4 m-0 p-0 list-none">
                                @foreach($assignment->exercise_config['hints'] as $hint)
                                    <li class="flex items-start gap-3 m-0 italic text-xs text-on-surface-variant">
                    ...
                    <div class="pt-6">
                        <div class="flex items-center gap-3 p-4 bg-surface-container rounded-2xl border border-outline-variant/10 italic">
            ...
            <section class="flex-1 flex flex-col bg-[#1e2029] relative">
                ...
                <div class="h-12 bg-[#191b23] border-b border-white/5 flex items-center justify-between px-6 shrink-0">
                    <div class="flex items-center gap-1 h-full">
                        <div class="h-full border-b-2 border-primary bg-[#282a36] px-4 flex items-center gap-2 group cursor-pointer">
                            ...
                            <span class="text-[10px] font-black text-on-primary-container italic uppercase tracking-widest">Main.{{ str_replace('mixed', '', data_get($assignment->exercise_config, 'language', 'html')) }}</span>
                        </div>
                        <div class="h-full px-4 flex items-center gap-2 opacity-40 hover:opacity-100 transition-opacity cursor-pointer text-white">
                            ...
                            <span class="text-[10px] font-bold italic uppercase tracking-widest">ReadMe.md</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <span class="text-[10px] font-bold text-outline-variant uppercase tracking-widest italic animate-pulse">Autosaving...</span>
                ...
                <div x-data="{ tab: 'preview' }" class="h-1/3 min-h-[200px] border-t border-white/10 bg-[#191b23] flex flex-col shrink-0">
                    ...
                    <div class="h-10 bg-[#1e2029] border-b border-white/5 flex items-center px-6 gap-6 shrink-0">
                        <button @click="tab = 'preview'" :class="tab === 'preview' ? 'text-primary border-b-2 border-primary italic' : 'text-outline-variant hover:text-white italic'" class="h-full text-[10px] font-black uppercase tracking-widest transition-all">OUTPUT PREVIEW</button>
                        <button @click="tab = 'logs'" :class="tab === 'logs' ? 'text-primary border-b-2 border-primary italic' : 'text-outline-variant hover:text-white italic'" class="h-full text-[10px] font-black uppercase tracking-widest transition-all">TERMINAL LOGS</button>
                    </div>
                    ...
                    <div class="flex-1 overflow-hidden">
                        ...
                        <div x-show="tab === 'logs'" class="h-full p-4 font-mono text-[11px] text-surface-dim space-y-1 overflow-y-auto custom-scrollbar">
                            ...
                            <p class="text-on-surface-variant/40 italic">&gt; Listening for code changes...</p>
                    ...
                    <div class="h-16 bg-[#1e2029] border-t border-white/10 px-6 flex items-center justify-between shrink-0">
                        <div class="flex items-center gap-4">
                            <button type="button" onclick="runCodePreview()" class="px-6 py-2 bg-white/5 text-white hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 italic">
                        ...
                        <div class="flex items-center gap-4">
                            @if(!$existing)
                                <form action="{{ route('mahasiswa.exercises.submit') }}" method="POST" id="submit-form">
                                    ...
                                    <button type="button" onclick="confirmSubmit()" class="px-10 py-2.5 bg-primary text-on-primary rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:shadow-2xl hover:shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2 italic shadow-lg shadow-black/20">
                                        SUBMIT SOLUTION
                                        <span class="material-symbols-outlined text-sm" style="font-variation-settings: 'FILL' 1;">send</span>
                                    </button>
                                </form>
                            @else
                                <div class="px-6 py-2.5 bg-secondary/10 text-secondary border border-secondary/20 rounded-xl text-[10px] font-black uppercase tracking-widest italic">
                                    SUBMITTED
                                </div>
                            @endif
                        </div>
