<x-app-layout>
 ...
 <tr>
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-on-surface-variant uppercase tracking-wider">Tugas/Quiz</th>
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-on-surface-variant uppercase tracking-wider hidden sm:table-cell">Tenggat Waktu</th>
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-on-surface-variant uppercase tracking-wider">Status</th>
 <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-on-surface-variant uppercase tracking-wider">Nilai</th>
 </tr>
 </thead>
 ...
 <a href="{{ route('mahasiswa.courses.show', $course) }}" class="hover:text-blue-600:text-blue-400 hover:underline">
 {{ $assignment->title }}
 </a> 
 <br><span class="text-xs text-outline">{{ ucfirst($assignment->type) }}</span>
 </td>
 <td class="px-6 py-4 whitespace-nowrap text-sm text-on-surface-variant hidden sm:table-cell">
 ...
 <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Dinilai</span>
 @elseif($submission && $submission->status === 'submitted')
 <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Menunggu Penilaian</span>
 @elseif($submission && $submission->status === 'late')
 <span class="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs">Terlambat</span>
 @else
 <span class="px-2 py-1 bg-surface-container-low text-on-surface text-on-surface rounded-full text-xs">Belum Mengumpulkan</span>
 @endif
 </td>
 <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-on-surface">
 {{ $score !== null ? $score : '-' }} <span class="text-xs font-normal text-outline">/ {{ $assignment->max_score ?? 100 }}</span>
 </td>
 </tr>
 @endforeach
 </tbody>
 ...
 <tr>
 <td colspan="2" class="sm:hidden px-6 py-3 text-right text-xs font-bold text-on-surface-variant uppercase tracking-wider">Rata-rata:</td>
 <td colspan="3" class="hidden sm:table-cell px-6 py-3 text-right text-xs font-bold text-on-surface-variant uppercase tracking-wider">Rata-rata Nilai:</td>
 <td class="px-6 py-3 text-left text-sm font-bold text-on-surface">
 ...
 <p class="mt-1 text-sm text-on-surface-variant">Anda belum mendaftar di mata kuliah apapun.</p>
 </div>
 @endforelse
 </div>
