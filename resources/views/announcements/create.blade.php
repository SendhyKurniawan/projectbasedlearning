<x-app-layout>
 ...
 <input type="checkbox" name="specific_users[]" value="{{ $user->id }}" class="rounded border-outline-variant/30 text-primary shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" {{ is_array(old('specific_users')) && in_array($user->id, old('specific_users')) ? 'checked' : '' }}>
 ...
 <textarea id="content" name="content" rows="8" class="border-outline-variant/30 text-on-surface focus:border-indigo-500:border-indigo-600 focus:ring-primary/20:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full" required>{{ old('content') }}</textarea>
