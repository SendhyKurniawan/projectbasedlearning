<x-guest-layout title="Masuk">
 ...
 id="password" name="password" dusk="password-input" type="password" :type="show ? 'text' : 'password'" placeholder="Enter your password" required autocomplete="current-password" />
 <button type="button" dusk="password-toggle" @click="show = !show" class="absolute right-4 top-1/2 -translate-y-1/2 text-outline hover:text-primary transition-colors focus:outline-none flex items-center justify-center">
