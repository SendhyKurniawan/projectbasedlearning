...
<x-app-layout>
 ...
 <form method="GET" action="{{ route('admin.grades.index') }}" class="flex flex-col md:flex-row md:items-end gap-5">
 ...
 <label for="dosen_id" class="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Dosen</label>
 <div class="relative focus-within:text-primary text-on-surface-variant">
 <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
 ...
 <a href="{{ route('admin.grades.index') }}" class="flex items-center justify-center gap-2 h-[42px] px-5 border border-outline-variant/30 bg-surface rounded-xl text-sm font-bold text-on-surface-variant hover:bg-surface-container transition-colors shadow-sm w-full md:w-auto">
 ...
 <span class="truncate max-w-[120px]">{{ Str::limit($assignment->title, 15) }}</span>
 </div>
 </th>
 @endforeach
 ...
 <p class="text-sm text-on-surface-variant max-w-md mt-2">Silakan ubah filter pencarian Anda, atau pastikan data sudah tersedia.</p>
 </div>
 @endforelse
 </div>
 </div>
