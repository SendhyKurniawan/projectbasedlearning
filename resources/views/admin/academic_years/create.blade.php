{{-- Form tambah tahun ajaran (admin). --}}
<x-app-layout>
 <div class="space-y-6">
 {{-- Section Header --}}
 ...
 <span class="material-symbols-outlined text-xs">chevron_right</span>
 <span class="text-primary">{{ isset($academicYear) ? 'Edit' : 'Tambah' }}</span>
 </nav>
 <h1 class="font-headline text-3xl font-extrabold tracking-tight text-on-surface">{{ isset($academicYear) ? 'Edit Tahun Akademik' : 'Tambah Tahun Akademik' }}</h1>
 <p class="mt-1 text-sm text-on-surface-variant">Tentukan rentang tahun ajaran dan status aktif.</p>
 </div>
 ...
 <form method="POST" action="{{ isset($academicYear) ? route('admin.academic-years.update', $academicYear) : route('admin.academic-years.store') }}">
 @csrf
 ...
 <input id="is_active" type="checkbox" class="rounded border-outline-variant/30 text-primary shadow-sm focus:ring-primary/20:ring-indigo-600:ring-offset-gray-800" name="is_active" {{ old('is_active', $academicYear->is_active ?? false) ? 'checked' : '' }}>
 ...
 <x-primary-button>
 {{ isset($academicYear) ? __('Update') : __('Simpan') }}
 </x-primary-button>
 </div>
 </form>
 </div>
